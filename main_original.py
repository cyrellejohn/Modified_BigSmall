...
def build_trainer(config, enable_ddp=False, precision='16-mixed', fast_dev_run=False, max_time=None,
                  val_check_interval=1/3, accumulate_grad_batches=1):
    """
    Builds a PyTorch Lightning Trainer based on GPU availability and whether DDP is enabled.
    
    Args:
        config: Configuration object.
        enable_ddp (bool): Set to True to enable DDP when multiple GPUs are available.
    ...
    if num_gpus == 0:
        ...
        return Trainer(accelerator="cpu",
    ...
    elif num_gpus == 1 or not enable_ddp:
        print("Running on Single GPU (or DDP disabled)")
        return Trainer(accelerator="gpu",
                       devices=1,
                       precision=precision,
                       fast_dev_run=fast_dev_run,
                       max_epochs=config.TRAIN.EPOCHS,
        ...
        print(f"Running on {num_gpus} GPUs with DDP")
        return Trainer(accelerator="gpu",
                       strategy='ddp',
                       precision=precision,
                       fast_dev_run=fast_dev_run,
