...
from utils.stratified_split import stratified_subject_split, get_raw_data
...
def build_dataloader(mode, data_config, batch_size, num_workers, shuffle, seed_worker, generator, subject_dirs):
    ...
    dataset = loader_class(mode, data_config.DATA_PATH, data_config, subject_dirs=subject_dirs)
...
def main():
    ...
    # Perform stratified subject split ONCE
    if config.TRAIN.DATA.DATASET == "MDMER":
        all_subject_dirs = get_raw_data(config.TRAIN.DATA.DATA_PATH, config.TRAIN.DATA.DATASET)
        train_dirs, val_dirs, test_dirs = stratified_subject_split(all_subject_dirs, valid_size=0.10, test_size=0.10, seed=17)
    else:
        train_dirs = val_dirs = test_dirs = None  # fallback for non-MDMER datasets
