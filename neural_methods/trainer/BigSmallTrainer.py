...
class BigSmallTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        self.config = config
        self.use_scaler = False # | WILL FULLY IMPLEMENT IN THE FUTURE: https://tinyurl.com/nhh532af
        ...
        au_label_list = [label for label in label_list if '_occ' in label]
        ppg_label_list_train = [label for label in label_list if label == 'pos_env_norm_ppg']
        ppg_label_list_test = [label for label in label_list if label == 'ppg']
    ...
    def train(self, data_loader):
        ...
        Description:
            ...
            It initializes and tracks various loss metrics for different tasks (AU, PPG) and updates the model's
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                au_output, ppg_output = self.model(data)
            ...
            if self.config.VALID.RUN_VALIDATION:
                valid_loss, valid_au_loss, valid_ppg_loss = self.valid(data_loader)
        ...
        # Print the epoch of the model to be used for testing
    ...
    def valid(self, data_loader):
        ...
        Description:
            ...
            It initializes and tracks various loss metrics for different tasks (AU, PPG) without updating
        ...
        Returns:
            tuple: A tuple containing the mean validation loss, mean AU loss and mean PPG loss
        """
        ...
        with torch.no_grad():
            ...
            for valid_idx, valid_batch in enumerate(vbar):
                ...
                au_out, ppg_out = self.model(data)
        ...
        return np.mean(valid_loss), np.mean(valid_au_loss), np.mean(valid_ppg_loss)
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for _, test_batch in enumerate(tqdm(data_loader["test"], ncols=80)):
                ...
                # Get predictions from the model for AU and PPG tasks
                au_out, ppg_out = self.model(data)
